...
void *startService( void *threadid )
{
   ...
   int rc;
   clockid_t clock;
   runInterference = intfTime * 100000;
   ...
   clock_gettime( clock, &timeNow);
   printf( "Low prio %d thread spawned at %ld sec, %ld nsec\n", LOW_PRIO_SERVICE, timeNow.tv_sec, timeNow.tv_nsec );
   ...
   clock_gettime( clock, &timeNow);
   printf( "Middle prio %d thread spawned at %ld sec, %ld nsec\n", MID_PRIO_SERVICE, timeNow.tv_sec, timeNow.tv_nsec );
   ...
   clock_gettime( clock, &timeNow);
   printf( "High prio %d thread spawned at %ld sec, %ld nsec\n", HIGH_PRIO_SERVICE, timeNow.tv_sec, timeNow.tv_nsec );
